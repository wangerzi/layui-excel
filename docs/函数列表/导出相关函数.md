...
| opt.writeOpt    | 写入函数的可选项覆盖，一般用默认生成的配置即可，具体请查看 [Writing Options](https://github.com/SheetJS/sheetjs#writing-options)  | undefined |

> writeOpt 中比较有意义的是 bookSST，可以开启 SharedString.xml 的写入，可加强某些导入场景的兼容性，代价是体积增加（如果开启了 opt.compression）
...
> 还需要注意的是，对象的 key 应该避免为数字，因为数字 key 的顺序在对象中不可控，容易导致乱序。
